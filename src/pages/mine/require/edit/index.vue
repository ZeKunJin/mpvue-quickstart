    ...
    <div class="container">
      <van-cell-group :border="false" >
        ...
        <div style="padding: 10px 16px; padding-top: 0">
          <div class="desc-block" v-if="showDesc">
            <van-icon name="clear" class="clear-icon" size="18" @click="delDesc" />

            <div v-html="descriptionContent" />
          </div>
          <editor @input="onTextInput" ref="editor" :value="requirement.description" />
        </div>
...
import { typeList, detailList, requirementDetail } from '@/api/require'
...
export default {
  ...
  methods: {
    ...
    async fetch () {
      try {
        const [orgRes, typeRes, requirementRes] = await Promise.all([
          userOrgList({
            page: 1,
            limit: 100,
        ...
        await Promise.all([this.initOrgColumns(orgRes.data.rows), this.initType(typeRes.data.rows)])
        this.initRequirement(requirementRes.data)
      } catch (error) {
        // on error
      }
    },

    async initRequirement (value) {
      const arr = []
      this.requirement = value
      this.descriptionContent = this.requirement.description
      this.requirement.description = ''
      this.files = this.requirement.isDefaultCover === '1' ? [{ url: value.cover }] : []
      for (const item of this.orgList) {
        if (item.value === value.orgId) {
      ...
      for (const item of this.typeList) {
        if (item.value = value.type) {
          ...
          this.typeChildrenTitle = item.text
          break
        }
      }
      await this.getTypeChildrenList(value.type)
      for (const item of this.typeChildrenList) {
        if (value.demandType.indexOf(item.value) > -1) {
          arr.push(item.text)
        }
      }
      this.typeChildrenText = arr.join(',')
    },
    ...
    async confirmType (event) {
      ...
      await this.getTypeChildrenList(this.requirement.type)
      this.showType = false
    },
    ...
    validate () {
      return new Promise((resolve, reject) => {
        ...
        if ( data.demandType &&
          (data.description || this.descriptionContent) &&
          data.level &&
          data.orgId &&
          data.title &&
    ...
    async next () {
      try {
        await this.validate()
        try {
          this.requirement.cover = this.files.length > 0 ? this.files[0].url : ''
          this.requirement.demandPeople = Number(this.requirement.demandPeople)
          this.requirement.description = this.descriptionContent + this.requirement.description
          this.$navigate.to('/pages/mine/require/region/index', {
...
.edit-require .bottom .van-button {
  box-shadow: 0 4px 8px 0 rgba(248, 110, 110, 0.25);
}

.edit-require .cancel-button .van-button {
  box-shadow: 0 6px 10px 0 rgba(228, 235, 248, 0.31);
}

.edit-require .next-button .van-button {
  box-shadow: 0 4px 8px 0 rgba(248, 110, 110, 0.25);
}

.edit-require .desc-block {
  padding: 10px 16px;
...
.edit-require .desc-block .clear-icon {
  position: absolute;
  top: 5px;
  right: 5px
}
