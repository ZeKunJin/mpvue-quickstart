      ...
      <van-row gutter="20">
        ...
        <van-col span="12" style="text-align: left" class="next-button">
          <van-button @click="next" round color="#E61F1C">
            <div style="width: 128px">立即发布</div>
          </van-button>
        </van-col>
      </van-row>
    </div>
...
<script>
import { requirementPublish } from "@/api/require";
import { regionInfo } from '@/api/region'
    ...
    async onRequire() {
      try {
        ...
        try {
          ...
          await requirementPublish(this.requirement);
          this.$navigate.to("/pages/require/index");
          wx.showToast({
            title: "发布成功",
            icon: "none"
          });
          this.$store.dispatch('requirement/remove')
        } catch (error) {
          // on error
        }
      } catch (error) {
